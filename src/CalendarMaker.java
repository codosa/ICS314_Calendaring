...
public class CalendarMaker {
	...
	public void createFreeTime(String[] filenames) {
			...
			if (validFile(filename)) {
				...
				if (start != null && end != null) {
					...
					if (dateBlocks.isEmpty() || (!dateBlocks.isEmpty() && 
							dateBlocks.get(0).getStartTime().isSameDate(block.getStartTime()))) {
		...
		int year = dateBlocks.get(0).getStartTime().getCal().get(Calendar.YEAR);
		// add one because cal.get(Calendar.MONTH) starts at 0 for jan
		int month = dateBlocks.get(0).getStartTime().getCal().get(Calendar.MONTH) + 1;
		int day = dateBlocks.get(0).getStartTime().getCal().get(Calendar.DAY_OF_MONTH);
	...
	public boolean grabStart() {
		...
		return dateBlock.getStartTime().grab();
	}
	
	public boolean grabEnd() {
		System.out.println("-------EndTime-------");
		return dateBlock.getEndTime().grab();
	}
	...
	public void showStartTime() {
		System.out.println("------StartTime------");
		dateBlock.getStartTime().show();
		System.out.println("---------------------");
	}
	
	public void showEndTime() {
		System.out.println("-------EndTime-------");
		dateBlock.getEndTime().show();
		System.out.println("---------------------");
	}
	...
	public void makeCalendar(String filename) {
		...
		writer.println("DTSTART;TZID=Pacific/Honolulu:" + dateBlock.getStartTime().format());
		writer.println("DTEND;TZID=Pacific/Honolulu:" + dateBlock.getEndTime().format());
